...
#define CRB_STACK_HEADER size_t e_size; unsigned int capacity, count; uint8_t *data

struct crb_stack {
	CRB_STACK_HEADER;
};

extern int crb_stack_init(void *trunk, size_t e_size, unsigned int capacity);
extern void crb_stack_destroy(void *trunk);

extern unsigned int crb_stack_append(void *trunk, void *elements, unsigned int count);
